...
private:
    double percent;
public:
    PercentageDiscountStrategy(double percent) {
...
// DiscountStrategyManager (Singleton Class)
...
// Product, Cart and CartItem Classes
...
// Coupon Manager (Singleton Class)
...
int main() {
    ...
    mgr->registerCoupon(new SeasonalOffer(15, "Electronics"));
    mgr->registerCoupon(new LoyaltyDiscount(7));
    mgr->registerCoupon(new BulkPurchaseDiscount(10000, 1000));
    mgr->registerCoupon(new BankingCoupon("GGG", 2000, 15, 500));
    ...
    Product* p4 = new Product("Headphones", "Electronics", 9000);
